module.exports = {
    ...
    transform: ({ file, root, api, options }) => {
        ...
        const createFindPropFn = prop => property =>
            property.key && property.key.type === 'Identifier' && property.key.name === prop;

        const filterDefaultPropsField = node => createFindPropFn(DEFAULT_PROPS_FIELD)(node);

        const filterGetInitialStateField = node => createFindPropFn(GET_INITIAL_STATE_FIELD)(node);
        ...
        const checkDeprecatedAPICalls = classPath =>
            DEPRECATED_APIS.reduce(
                (acc, name) => acc + j(classPath).find(j.Identifier, { name }).size(),
                0
            ) > 0;
        ...
        const doesNotUseArguments = classPath => {
            const hasArguments = j(classPath).find(j.Identifier, { name: 'arguments' }).size() > 0;
            if (hasArguments) {
                console.warn(
                    file.path +
                        ': `' +
        ...
        const collectNonStaticProperties = specPath =>
            specPath.properties
                .filter(
                    prop => !(filterDefaultPropsField(prop) || filterGetInitialStateField(prop))
                )
                .filter(
                    prop =>
                        !STATIC_KEYS.hasOwnProperty(prop.key.name) && prop.key.name !== STATIC_KEY
                )
                .filter(
                    prop =>
                        isFunctionExpression(prop) ||
        ...
        const flowFixMeType = j.genericTypeAnnotation(j.identifier('$FlowFixMe'), null);
        ...
        const createESClass = (
            ...
            return withComments(
                j.classDeclaration(
                    ...
                    j.memberExpression(j.identifier('React'), j.identifier(baseClassName), false)
                ),
                { comments }
            );
        };
        ...
        const createStaticClassProperties = statics => statics.map(createStaticClassProperty);
        ...
        const findUnusedVariables = (path, varName) =>
            j(path)
                ...
                .filter(identifierPath => {
                    ...
                    return !(j.MemberExpression.check(parent) &&
        ...
        const updateToClass = classPath => {
            ...
            const baseClassName = pureRenderMixinPathAndBinding &&
        ...
        const addDisplayName = (displayName, specPath) => {
            ...
            if (safe) {
                props.unshift(
                    j.objectProperty(j.identifier('displayName'), j.stringLiteral(displayName))
                );
            }
        };
        ...
        if (options['explicit-require'] === false || ReactUtils.hasReact(root)) {
            ...
            if (didFallback) {
                const reactPathAndBinding =
                    findRequirePathAndBinding('react') || findRequirePathAndBinding('React');
